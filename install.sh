...
get1=$(awk -F'/' 'FNR==5 {print $2}' cfg.md | awk -F'G]' '{gsub(/\./,$1); print $1}') && printf '%.*f\n' 0 $get1 | awk '{print $1*1000/4}' > $TEMP_FILE && buffers=$(awk '{print $1}' $TEMP_FILE) && sudo sed -i "s|shared_buffers = 128MB|shared_buffers = $buffers|g" $POSTGRES
sudo sed -i "s|#temp_buffers = 8MB|temp_buffers = 256MB|g" $POSTGRES
get2=$(awk -F'/' 'FNR==5 {print $2}' cfg.md | awk -F'G]' '{gsub(/\./,$1); print $1}') && printf '%.*f\n' 0 $get2 | awk '{print $1*1000/32}' > $TEMP_FILE &&  mem=$(awk '{print $1}' $TEMP_FILE) && sudo sed -i "s|#work_mem = 4MB|work_mem = $mem|g" $POSTGRES
...
a="4" && get3=$(awk -F'thr; ' 'FNR==4 {print $2}' cfg.md | awk -F' running' '{gsub(/\./,$1); print $1}') && printf '%.*f\n' 0 $get3 | awk '{print $1/2}' > $TEMP_FILE && b=$(awk '{print $1}' $TEMP_FILE)
if [ "$a" -gt "$b" ]; then
...
a="4" && get4=$(awk -F'thr; ' 'FNR==4 {print $2}' cfg.md | awk -F' running' '{gsub(/\./,$1); print $1}') && printf '%.*f\n' 0 $get4 | awk '{print $1/2}' > $TEMP_FILE && b=$(awk '{print $1}' $TEMP_FILE)
if [ "$a" -gt "$b" ]; then
  sudo sed -i "s|#autovacuum_max_workers = 3|autovacuum_max_workers = $a|g" $POSTGRES
